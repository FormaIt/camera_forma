...
// FEHMİ SİLDİ import com.pedro.rtplibrary.util.RecordController
// FEHMİ EKLEDİ
...
// FEHMİ SİLDİ import net.ossrs.rtmp.SrsFlvMuxer
...
class RtmpCameraConnector(val context: Context, val useOpenGL: Boolean, val isPortrait: Boolean, val connectChecker: ConnectCheckerRtmp) :
    ...
    private var srsFlvMuxer: RtmpClient
    ...
    protected var audioInitialized: Boolean = false;
...
//        this.glInterface.setPreviewSize(videoEncoder!!.width, videoEncoder!!.height)
    ...
    fun startStreamRtp(url: String) {
        ...
        // FEHMİ SİLDİ 1srsFlvMuxer.start(url)
        // ÜÇ SATIRIDA FEHMİ EKLEDİ
        srsFlvMuxer.setFps(videoEncoder!!.fps);
        srsFlvMuxer.setOnlyVideo(!audioInitialized);
...
//        if (isRecording && !pausedRecording) recordController.recordVideo(h264Buffer, info)
    ...
    fun onSpsPpsVpsRtp(sps: ByteBuffer, pps: ByteBuffer, vps: ByteBuffer?) {
       ...
       srsFlvMuxer.setVideoInfo(sps, pps, vps);
    }
