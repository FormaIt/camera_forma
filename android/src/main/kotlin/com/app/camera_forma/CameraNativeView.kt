        ...
        if (!rtmpCamera.isStreaming) {
            try {
                if (rtmpCamera.prepareAudio() && rtmpCamera.prepareVideo()) {
                    Log.d("CameraNativeView Start record", "startVideoRecording filePath: $filePath")
                    rtmpCamera.startRecord(filePath, null)
                    result.success(null)
                } else {
                    result.error("videoStreamingFailed", "Prepare olmadı", null)    
                }
            ...
            // FEHMİ ELSE'İ EKLEDİ
            Log.d("FEHMİ Start record", "startVideoRecording filePath: $filePath")
            try {
                rtmpCamera.startRecord(filePath, null)
                result.success(null)
            } catch (e: CameraAccessException) {
