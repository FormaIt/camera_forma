...
import com.app.camera_forma.CameraPermissions.ResultCallback
...
//    private var camera: CameraWrapper? = null
    ...
    init {
        methodChannel = MethodChannel(messenger, "plugins.flutter.io/camera_forma")
        imageStreamChannel = EventChannel(messenger, "plugins.flutter.io/camera_forma/imageStream")
...
//                camera?.takePicture(call.argument("path")!!, result)
            ...
            "prepareForVideoRecording" -> {
                Log.d("Stuff", "prepareForVideoRecording")
                // This optimization is not required for Android.
                result.success(null)
            }
            "startVideoRecording" -> {
                Log.d("Stuff", "startVideoRecording")
                camera?.startVideoRecording(call.argument("filePath")!!, result)
            }
            "startVideoStreaming" -> {
                Log.d("Stuff", "startVideoStreaming ${call.arguments.toString()}")
            ...
            "startVideoRecordingAndStreaming" -> {
                Log.d("Stuff", "startVideoRecordingAndStreaming ${call.arguments.toString()}")
                var bitrate: Int? = null
                if (call.hasArgument("bitrate")) {
                    bitrate = call.argument("bitrate")
                }
